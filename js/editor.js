...
const Editor = {
  ...
  /* 进入管理模式：先验证权限（本地服务密码） */
  async toggle() {
    if (App.readonly) { toast("当前为云端只读版，请用「启动网站.bat」打开本地版进行管理", "err"); return; }
    if (Editor.mode) { Editor.exit(); return; }
    if (sessionStorage.getItem("adminAuthed") === "1") { Editor.enter(); return; }
    Editor.askPassword();
  },
  ...
  async verify(pwd) {
    try {
      ...
      if (j && j.ok) {
        ...
        if (j.needSet) toast("管理模式已开启（尚未设置密码，建议在管理面板中设置）", "ok");
        else toast("🛠 管理模式已开启", "ok");
        Editor.enter();
        return true;
      }
      toast("密码错误，无法进入管理模式", "err");
      return false;
    } catch (e) { toast("验证失败：无法连接本地服务", "err"); return false; }
  },

  askPassword() {
    document.getElementById("modalTitle").textContent = "🔒 管理模式验证";
    const form = document.getElementById("modalForm");
    form.innerHTML = `
      <div class="form-row"><label>管理密码（仅后台人员可进入，访客不可修改）</label>
        <input type="password" class="form-input" id="adminPwd" placeholder="请输入管理密码" autocomplete="off"></div>
      <div class="form-actions">
        <button type="button" class="btn btn-ghost" onclick="Editor.closeModal()">取消</button>
        <button type="button" class="btn btn-primary" onclick="Editor.verify(document.getElementById('adminPwd').value)">进入管理模式</button></div>`;
    document.getElementById("modal").classList.remove("hidden");
    setTimeout(() => { const i = document.getElementById("adminPwd"); if (i) i.focus(); }, 60);
  },
  ...
  openPanel() {
    ...
    const row = el(`<div class="form-actions" style="border-top:1px dashed var(--line);margin-top:20px;padding-top:14px">
      <button type="button" class="btn btn-sm" onclick="Editor.downloadBackup()">⬇ 下载数据备份</button>
      <button type="button" class="btn btn-sm" onclick="Editor.exportSite()">🚀 导出静态版（上传云端）</button>
      <button type="button" class="btn btn-sm" onclick="Editor.setPassword()">🔑 设置管理密码</button>
  ...
  /* 设置管理密码（本地保存，导出云端时自动清除） */
  setPassword() {
    document.getElementById("modalTitle").textContent = "🔑 设置管理密码";
    const form = document.getElementById("modalForm");
    form.innerHTML = `
      <div class="form-row"><label>新管理密码</label>
        <input type="password" class="form-input" id="np1" placeholder="设置后进入管理模式需输入此密码" autocomplete="new-password"></div>
      <div class="form-row"><label>再次输入确认</label>
        <input type="password" class="form-input" id="np2" placeholder="重复输入一次" autocomplete="new-password"></div>
      <div class="form-hint" style="margin-bottom:14px">💡 密码仅保存在本地 data.json，导出的云端版会自动清除密码；访客无法修改内容。</div>
      <div class="form-actions">
        <button type="button" class="btn btn-ghost" onclick="Editor.closeModal()">取消</button>
        <button type="button" class="btn btn-primary" onclick="Editor.savePassword()">保存密码</button></div>`;
    document.getElementById("modal").classList.remove("hidden");
  },
  savePassword() {
    const p1 = document.getElementById("np1").value, p2 = document.getElementById("np2").value;
    if (!p1) { toast("请输入密码", "err"); return; }
    if (p1 !== p2) { toast("两次输入的密码不一致", "err"); return; }
    App.DATA.settings.adminPassword = p1;
    Editor.closeModal();
    App.save();
    toast("🔑 管理密码已设置，下次进入需输入", "ok");
  },
